...
public class LavaSrcPlugin implements AudioPlayerManagerConfiguration, SearchManagerConfiguration, LyricsManagerConfiguration {
    ...
    this.deezer = new DeezerAudioSourceManager(
        deezerConfig.getMasterDecryptionKey(),
        deezerConfig.getArl(),
...
if (deezerConfig != null && this.deezer != null) {
    if (deezerConfig.getArl() != null && !deezerConfig.getArl().isEmpty()) {
        log.info("Updating Deezer ARL...");
        this.deezer.setArl(deezerConfig.getArl());
    } else {
        log.warn("Received an empty or null Deezer ARL, ignoring update.");
    }
}
